...
(function() {
  ...
  app.controller("QuestController", function(
    $rootScope,
    $scope,
    $http,
    $stateParams,
    ...
    if ($stateParams.id) {
      ...
      $http
        .get(app.api + "quest/template/" + $stateParams.id)
        .success(function(data, status, header, config) {
          $scope.current_quest_template = $rootScope.fixNumericValues(data[0]);
          $scope.new_quest_template = angular.copy(
            $scope.current_quest_template
          );
          $scope.selectionText =
            $scope.current_quest_template.LogTitle +
            " (" +
            $scope.current_quest_template.ID +
            ") ";
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] quest/template/" +
              $stateParams.id +
      ...
      $http
        .get(app.api + "quest/template/addon/" + $stateParams.id)
        .success(function(data, status, header, config) {
          $scope.current_quest_template_addon = $rootScope.fixNumericValues(
            data[0]
          );
          $scope.new_quest_template_addon = angular.copy(
            $scope.current_quest_template_addon
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] quest/template/addon/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      $http
        .get(app.api + "quest/details/" + $stateParams.id)
        .success(function(data, status, header, config) {
      ...
      $http
        .get(app.api + "quest/offer_reward/" + $stateParams.id)
        .success(function(data, status, header, config) {
          $scope.current_quest_offer_reward = $rootScope.fixNumericValues(
            data[0]
          );
          $scope.new_quest_offer_reward = angular.copy(
            $scope.current_quest_offer_reward
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] quest/offer_reward/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      $http
        .get(app.api + "quest/request_items/" + $stateParams.id)
        .success(function(data, status, header, config) {
          $scope.current_quest_request_items = $rootScope.fixNumericValues(
            data[0]
          );
          $scope.new_quest_request_items = angular.copy(
            $scope.current_quest_request_items
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] quest/request_items/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      $http
        .get(app.api + "creature/queststarter/quest/" + $stateParams.id, {
          params: { names: 0 }
        })
        .success(function(data, status, header, config) {
          $scope.current_creature_queststarter = $rootScope.fixNumericValues(
            data
          );
          $scope.new_creature_queststarter = angular.copy(
            $scope.current_creature_queststarter
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] creature/queststarter/quest/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      $http
        .get(app.api + "creature/questender/quest/" + $stateParams.id, {
          params: { names: 0 }
        })
        .success(function(data, status, header, config) {
          $scope.current_creature_questender = $rootScope.fixNumericValues(
            data
          );
          $scope.new_creature_questender = angular.copy(
            $scope.current_creature_questender
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] creature/questender/quest/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      $http
        .get(app.api + "gameobject/queststarter/quest/" + $stateParams.id, {
          params: { names: 0 }
        })
        .success(function(data, status, header, config) {
          $scope.current_gameobject_queststarter = $rootScope.fixNumericValues(
            data
          );
          $scope.new_gameobject_queststarter = angular.copy(
            $scope.current_gameobject_queststarter
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] gameobject/queststarter/quest/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      $http
        .get(app.api + "gameobject/questender/quest/" + $stateParams.id, {
          params: { names: 0 }
        })
        .success(function(data, status, header, config) {
          $scope.current_gameobject_questender = $rootScope.fixNumericValues(
            data
          );
          $scope.new_gameobject_questender = angular.copy(
            $scope.current_gameobject_questender
          );
        })
        .error(function(data, status, header, config) {
          console.log(
            "[ERROR] gameobject/questender/quest/" +
              $stateParams.id +
              " $http.get request failed"
          );
        });
      ...
      if ($rootScope.$storage.version.name == "6.x") {
        ...
        $http
          .get(app.api + "quest/objectives/" + $stateParams.id)
          .success(function(data, status, header, config) {
            $scope.current_quest_objectives = $rootScope.fixNumericValues(data);
            $scope.new_quest_objectives = angular.copy(
              $scope.current_quest_objectives
            );
          })
          .error(function(data, status, header, config) {
      ...
      $scope.selectionText =
        "No Quest selected. Please use Search to select one.";
    }
    ...
    $scope.search = function(questId, questTitle) {
      ...
      $http
        .get(app.api + "search/quest/" + questId + "/" + questTitle)
        .success(function(data, status, header, config) {
    ...
    $scope.generateQuestScript = function() {
      ...
      $scope.questScript += app.getUpdateQuery(
        "quest_template",
        whereCondition,
        $scope.current_quest_template,
        $scope.new_quest_template
      );
      ...
      $scope.questScript += app.getUpdateQuery(
        ...
        whereCondition,
        $scope.current_quest_template_addon,
        $scope.new_quest_template_addon
      );

      $scope.questScript += app.getUpdateQuery(
        "quest_details",
        whereCondition,
        $scope.current_quest_details,
        $scope.new_quest_details
      );
