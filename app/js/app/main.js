...
(function() {
  ...
  app.run(function($rootScope, $uibModal, $stateParams, $localStorage, $http) {
    ...
    $rootScope.isEntrySelected = function() {
      if (
        $stateParams.id ||
        ($stateParams.entryOrGuid && $stateParams.sourceType)
      ) {
        return true;
      } else {
     ...
     */
    $rootScope.openFlagModal = function(
      TemplateUrl,
      object,
      property,
      numValues
     ...
     */
    $rootScope.openBigValueModal = function(
      object,
      property,
      constant,
      modifier
    ...
    $rootScope.openDiffScriptModal = function(
      tableName,
      primaryKey1,
      primaryKey2,
      currentRows,
    ...
    $rootScope.openDiffOneKeyScriptModal = function(
      tableName,
      primaryKey,
      entityType,
      entity,
    ...
    $rootScope.POST_DeleteInsert = function(tableName, primaryKeys, Rows) {
      ...
      $http({
        method: "POST",
        url: location.origin + "/TC_API/DeleteInsert",
        data: json,
        headers: { "Content-Type": "text/plain" }
      }).success(function(data) {});
    };

    $rootScope.POST_UpdateQuery = function(
      API_URI,
      tableName,
      currentRow,
      newRow
